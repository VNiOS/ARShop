...
- (id) initWithAnnotation:(id <MKAnnotation,UITableViewDelegate,UITableViewDataSource>)annotation reuseIdentifier:(NSString *)reuseIdentifier {
...
- (void)setAnnotation:(id <MKAnnotation,UITableViewDelegate,UITableViewDataSource>)annotation {
	[super setAnnotation:annotation];
    [self  setAnnotationTableView:annotation];
...
- (void)prepareFrameSize {
	...
	frame.size = CGSizeMake(220, height);
	self.frame = frame;
}
...
- (UIView *)contentView {
	if (!_contentView) {
        ...
        UITableView *tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 5, 200, 50) style:UITableViewStylePlain];
        
        [tableView.layer setCornerRadius:5];
        tableView.dataSource = self.annotationTableView;
        tableView.delegate  = self.annotationTableView;
        NSLog(@"creat tableview");
